...
local auto_backslash_snippet = require("luasnippets.tex.utils.scaffolding").auto_backslash_snippet
local symbol_snippet = require("luasnippets.tex.utils.scaffolding").symbol_snippet
local single_command_snippet = require("luasnippets.tex.utils.scaffolding").single_command_snippet
local postfix_snippet = require("luasnippets.tex.utils.scaffolding").postfix_snippet
...
local M = {
    ...
    autosnippet({ trig = "(^.*\\))/", name = "fraction", dscr = "auto fraction 2", trigEngine = "ecma" }, { d(1, generate_fraction) }, { condition = tex.in_math, show_condition = tex.in_math }),
...
for k, v in pairs(greek_specs) do
    table.insert(greek_snippets, symbol_snippet(vim.tbl_deep_extend("keep", { trig = k }, v.context), v.command, { condition = tex.in_math, backslash = true }))
end
...
for k, v in pairs(symbol_specs) do
    table.insert(symbol_snippets, symbol_snippet(vim.tbl_deep_extend("keep", { trig = k }, v.context), v.command, { condition = tex.in_math }))
end
...
for k, v in pairs(single_command_math_specs) do
    table.insert(
        ...
        single_command_snippet(vim.tbl_deep_extend("keep", { trig = k, snippetType = "autosnippet" }, v.context), v.command, { condition = tex.in_math }, v.ext or {})
    )
end
...
for k, v in pairs(postfix_math_specs) do
    table.insert(postfix_math_snippets, postfix_snippet(vim.tbl_deep_extend("keep", { trig = k, snippetType = "autosnippet" }, v.context), v.command, { condition = tex.in_math }))
end
