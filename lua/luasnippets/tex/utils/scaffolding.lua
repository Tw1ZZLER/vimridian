...
M.auto_backslash_snippet = function(context, opts)
    opts = opts or {}
    if not context.trig then error("context doesn't include a `trig` key which is mandatory", 2) end
...
M.single_command_snippet = function(context, command, opts, ext)
    ...
    return s(context, fmta(command .. [[<>{<>}<><>]], { cnode or t "", i(1 + (offset or 0)), (lnode or t ""), i(0) }), opts)
end
